...
def modulador(bits, fc, mpp): 
    ...
    #Se defienen las portadoras
    portadora1 = np.cos(2*np.pi*fc*t_periodo)
    portadora2 = np.sin(2*np.pi*fc*t_periodo)
    #Se obtiene el tiempo total de la simulacion
    t_simulacion = np.linspace(0, (len(bits))*Tc, (len(bits))*mpp)
    #Se define un vector de cero para darle soporte a la grafica 
    senal_Tx = np.zeros(t_simulacion.shape)
    #Se hace un for en el cual se van comparando paquetes de dos bits igual que como dice la constelacion
    for i in range(0, len(bits), 4):
        senal_Tx[iterador*mpp : (iterador+1)*mpp] =             (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portadora1             + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portadora2
        iterador += 1
    #Se aplica la fomula
...
def demodulador(senal_Rx, portadora1, portadora2, mpp):
    M = len(senal_Rx) #Numero total de muestras
    N = int(M / mpp)  #Numero total de transmision
    bits_Rx = np.zeros(N)
    senal_demodulada = np.zeros(senal_Rx.shape)
    iterador = 0
    ...
    for i in range(N):
        if iterador+4 > N:  
            ...
            #Se detectan los bits
            if np.sum(producto1) >= 0: bits_Rx[iterador] = 1  # bit numero 1 
            if np.max(np.abs(producto1)) < 2.5: bits_Rx[iterador+1] = 1  # bit numero 2
            if np.sum(producto2) < 0: bits_Rx[iterador+2] = 1  # bit numero 3
            if np.max(np.abs(producto2)) < 2.5: bits_Rx[iterador+3] = 1  # bit numero 4
...
def show(img):
    ...
    #Se importa la imagen, y se dimensiona
...
# Se van a imprimir las imagenes
...
# 
# 
# Lo único que afecta es que se recurre a menos bits para transmitir la señal.

# In[15]:


#Se hace una prueba extra con una foto en resolución de 1920x1080
show('prueba.png')


# Como se puede obsevar entre más grande sea la imagen, los errores se notan menos. 
